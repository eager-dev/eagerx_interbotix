...
# NAME = "2023-02-22-1526_0.1_0.2_no_ori"
STEPS = 1_600_000
# STEPS = 1_025_000
# STEPS = 275_000
...
if __name__ == "__main__":
    ...
    graph.disconnect(source=vx300s.sensors.ee_pos, observation="ee_position")
    graph.disconnect(source=vx300s.sensors.position, observation="joints")
    graph.disconnect(source=vx300s.sensors.velocity, observation="velocity")
    graph.disconnect(source=vx300s.sensors.position, target=safe.inputs.position)
    graph.disconnect(source=vx300s.sensors.velocity, target=safe.inputs.velocity)
    graph.disconnect(source=vx300s.sensors.ee_pos, target=ik.inputs.xyz)
    graph.disconnect(source=vx300s.sensors.ee_orn, target=ik.inputs.orn)
    vx300s.config.sensors = ["position", "velocity", "force_torque"]

    # Add smith predictor
    from eagerx_interbotix.smith.node import SmithPredictor
    from eagerx_interbotix.xseries.mr_descriptions import vx300s as mr

    cfg = vx300s.config
    smith = SmithPredictor.make("smith_predictor", rate=rate, Slist=mr.Slist.tolist(), M=mr.M.tolist(), delays=0, motor_constants=0.75, lower=cfg.joint_lower, upper=cfg.joint_upper, vel_limit=cfg.vel_limit)
    graph.add(smith)
    smith.outputs.ee_pos.space = vx300s.sensors.ee_pos.space

    graph.connect(source=safe.outputs.filtered, target=smith.inputs.vel_command, skip=True)
    graph.connect(source=smith.outputs.ee_orn, target=ik.inputs.orn)
    graph.connect(source=smith.outputs.ee_pos, target=ik.inputs.xyz)
    graph.connect(source=smith.outputs.pos, target=safe.inputs.position)
    graph.connect(source=smith.outputs.vel, target=safe.inputs.velocity)
    graph.connect(source=vx300s.sensors.position, target=smith.inputs.pos, window=round(delay * rate) + 2)
    graph.connect(source=vx300s.sensors.velocity, target=smith.inputs.vel)
    graph.connect(source=smith.outputs.ee_pos, observation="ee_position")
    graph.connect(source=smith.outputs.pos, observation="joints")
    graph.connect(source=smith.outputs.vel, observation="velocity")
    # pos_predicted = vel_command_msg * self.delays + pos_msg + (vel_command_msg - vel_msg) * self.motor_constants * (np.exp(-self.delays / self.motor_constants) - 1)

    safe.config.vel_limit = [x * 0.5 for x in safe.config.vel_limit]
    ...
    if reset:
        ...
        graph.connect(source=smith.outputs.ee_pos, target=reset_node.inputs.ee_position)
        graph.connect(source=smith.outputs.ee_orn, target=reset_node.inputs.ee_orientation)
    if sim:
        graph.disconnect(source=safe.outputs.filtered, target=vx300s.actuators.vel_control)
        graph.connect(source=safe.outputs.filtered, target=vx300s.actuators.vel_control, delay=delay)
