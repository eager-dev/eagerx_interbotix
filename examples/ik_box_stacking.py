...
if __name__ == "__main__":
    ...
    LOG_DIR = ROOT + f"/logs/{datetime.today().strftime('%Y-%m-%d-%H%M')}_stacking_{delay_min}_{delay_max}_no_ori"
    add_bias = True
    excl_z = False
    T_max = 10.0  # [sec]
    rate = 10
    ...
    graph.connect(source=safe.outputs.filtered, observation="dtarget", window=ceil(delay_max * rate), skip=True)
    graph.gui()
